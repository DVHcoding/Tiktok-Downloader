...
def extract_id(
    ...
    def apply(cls: VideoInfo, url: str) -> VideoInfo:
        ...
        if subdo and subdo[0] in subdo_redirect:
            url = get(url, allow_redirects=False).text
...
class Author:
    def __init__(self, ascp: dict):
        ...
        self.create_time = (
            datetime.fromtimestamp(ascp["create_time"]) if ascp["create_time"] else None
        )
...
class VideoInfoAsync(AsyncClient):
    def __init__(self, js: dict, id: str):
        ...
        self.id = id
        self.aweme = js
        self.height = js["aweme_detail"]["video"]["download_addr"]["height"]
        self.width = js["aweme_detail"]["video"]["download_addr"]["width"]
        self.size = js["aweme_detail"]["video"]["download_addr"]["data_size"]
        self.desc = js["aweme_detail"]["desc"]
        self.cover = js["aweme_detail"]["video"]["origin_cover"]["url_list"][0]
        self.create_time = datetime.fromtimestamp(js["aweme_detail"]["create_time"])
        ...
        self.duration = int(js["aweme_detail"]["video"]["duration"] / 1000)
    ...
    def utils(self) -> list[DownloadAsync]:
        return [
            DownloadAsync(self.downloadLink(False), self),
            DownloadAsync(self.downloadLink(True), self, watermark=True),
            DownloadAsync(
                self.aweme["aweme_detail"]["music"]["play_url"]["uri"], self, "music"
            ),
        ]
...
class VideoInfo(Session):
    ...
    def utils(self) -> list[Download]:
        return [
            Download(self.downloadLink(False), self),
            Download(self.downloadLink(True), self, watermark=True),
            Download(
                self.aweme["aweme_detail"]["music"]["play_url"]["uri"], self, "music"
            ),
        ]
    ...
    def download(
        self, out: Optional[str] = None, watermark: bool = False, chunk_size: int = 1024
    ...
    def download_music(
        self, out: Optional[str] = None, chunk_size: int = 1024
    ) -> Union[None, BytesIO, BufferedWriter]:
        request = self.get(
            self.aweme["aweme_detail"]["music"]["play_url"]["uri"], stream=True
        )
        stream = open(out, "wb") if isinstance(out, str) else BytesIO()
        for i in request.iter_content(chunk_size):
            stream.write(i)
